...
interface Schedule1StatusChartProps {
  applications: BuildingConstructionApplication[];
...
export const Schedule1StatusChart: React.FC<Schedule1StatusChartProps> = ({ 
  applications, 
  onStatusClick,
  onActivityClick
}) => {
  const [activeTab, setActiveTab] = useState<'trend' | 'distribution' | 'activity' | 'ward'>('trend');
  const [timeRange, setTimeRange] = useState<TimeRangeOption>('30d');
  const [chartSubtype, setChartSubtype] = useState<'area' | 'bar'>('area');

  const submittedCount = applications.filter((a) => a.status === 'submitted' || a.status === 'pending').length;
  const underReviewCount = applications.filter((a) => a.status === 'under_review').length;
  const approvedCount = applications.filter((a) => a.status === 'approved').length;
  const rejectedCount = applications.filter((a) => a.status === 'rejected').length;
  const total = applications.length;

  const totalFeeAmount = applications.reduce((sum, a) => sum + (a.feeAmount || 1000), 0);
  const treasuryVerifiedCount = applications.filter((a) => Boolean(a.chalanOrDraftNo || a.treasuryCode)).length;
  ...
  const statusData = [
    ...
    {
      name: 'ট্রেজারী ও সাইট রিভিউাধীন',
      shortName: 'রিভিউাধীন',
  ...
  const activityData = useMemo(() => {
    ...
    applications.forEach((a) => {
      const type = (a.activityType || a.activityTypeTitle || a.natureOfWork?.typeOfWork || '').toString();
      if (type.includes('পুকুর')) counts['পুকুর খনন']++;
      else if (type.includes('পাহাড়')) counts['পাহাড় কর্তন']++;
      else if (type.includes('সীমানা')) counts['সীমানা প্রাচীর']++;
      else if (type.includes('ভবন') || type.includes('ইমারত')) counts['বহুতল ভবন']++;
    ...
    return Object.entries(counts).map(([name, count]) => ({
      ...
      color: name.includes('ভবন') ? '#059669' : name.includes('পুকুর') ? '#0284c7' : name.includes('পাহাড়') ? '#d97706' : name.includes('সীমানা') ? '#8b5cf6' : '#64748b'
    }));
  }, [applications]);
  ...
  const wardData = useMemo(() => {
    ...
    return wards.map((w) => {
      const count = applications.filter((a) => {
        const wardStr = (a.wardNo || a.plotDetails?.wardNo || '').toString();
        return wardStr.includes(w);
      }).length;
      return {
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 sm:p-6 space-y-5">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-base sm:text-lg font-bold text-slate-900 flex items-center gap-2">
              <span>তফসিল-১ ড্যাশবোর্ড অ্যানালিটিক্স ও আবেদন পরিসংখ্যান</span>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              ইমারত নির্মাণ অনুমোদন সংক্রান্ত আবেদনের রিয়েল-টাইম চার্ট ও পরিসংখ্যান
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 bg-slate-100 p-1.5 rounded-xl border border-slate-200 flex-wrap">
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
              activeTab === 'trend' ? 'bg-amber-600 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
              activeTab === 'distribution' ? 'bg-amber-600 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
              activeTab === 'activity' ? 'bg-amber-600 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
              activeTab === 'ward' ? 'bg-amber-600 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        >
          ...
          <span className="text-[10px] text-amber-700">ট্রেজারী এন্ট্রি বাকি</span>
        </div>
        ...
        >
          <span className="text-[11px] text-blue-800 font-bold block">রিভিউ ও তদন্তাধীন</span>
          <span className="text-xl font-black text-blue-950 block mt-0.5">{toBanglaNumber(underReviewCount)}</span>
          <span className="text-[10px] text-blue-700">নক্সাকার পরিদর্শনে</span>
        </div>
        ...
        >
          <span className="text-[11px] text-emerald-800 font-bold block">চূড়ান্ত অনুমোদিত</span>
          <span className="text-xl font-black text-emerald-950 block mt-0.5">{toBanglaNumber(approvedCount)}</span>
          <span className="text-[10px] text-emerald-700">তফসিল-১ অনুমতিপত্র ইস্যু</span>
        </div>

        <div className="p-3 bg-indigo-50/70 border border-indigo-200 rounded-xl">
          <span className="text-[11px] text-indigo-800 font-bold block">মোট সরকারি ফি আদায়</span>
          <span className="text-xl font-black text-indigo-950 block mt-0.5">৳ {toBanglaNumber(totalFeeAmount)}/-</span>
          <span className="text-[10px] text-indigo-700">চালান এন্ট্রিকৃত: {toBanglaNumber(treasuryVerifiedCount)} টি</span>
        </div>
      </div>
                ...
                {timeRangeOptions.map((opt) => (
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-md text-[11px] font-bold cursor-pointer transition-all ${
                      timeRange === opt.id ? 'bg-amber-600 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'
                    }`}
                  >
                    {opt.label}
                  </button>
            ...
            <ResponsiveContainer width="100%" height="100%">
              {chartSubtype === 'area' ? (
                <AreaChart data={trendData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  <defs>
                    <linearGradient id="colorIncoming1" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="#f59e0b" stopOpacity={0.4}/>
                      <stop offset="95%" stopColor="#f59e0b" stopOpacity={0}/>
                    </linearGradient>
                    <linearGradient id="colorCompleted1" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <Area type="monotone" dataKey="incoming" stroke="#f59e0b" fillOpacity={1} fill="url(#colorIncoming1)" name="incoming" strokeWidth={2} />
                  <Area type="monotone" dataKey="completed" stroke="#059669" fillOpacity={1} fill="url(#colorCompleted1)" name="completed" strokeWidth={2} />
                </AreaChart>
              ) : (
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 pt-2">
            {activityData.map((act) => (
              <div 
                key={act.name}
                onClick={() => onActivityClick && onActivityClick(act.name)}
              ...
              >
                <span className="text-[11px] font-bold text-slate-700 block truncate">{act.name}</span>
                <span className="text-base font-black text-slate-900 mt-0.5 block">{act.banglaCount} টি</span>
              </div>
            ))}
          </div>
        </div>
